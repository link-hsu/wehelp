...
def query_username():
    ...
    query_member_data = cursor.fetchone()
    cursor.close()
    if query_member_data:
        return jsonify({"data": query_member_data})
    else:
...
def update_name():
    if "member_id" in session:
        ...
        if update_name:
            cursor = con.cursor()
            cursor.execute("UPDATE member SET name = %s WHERE id = %s", (update_name, session["member_id"]))
            con.commit()           
            response = {"ok": True}
        else:
            response = {"error": True}
